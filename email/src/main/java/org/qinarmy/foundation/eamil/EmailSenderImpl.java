...
public class EmailSenderImpl implements EmailSender, EnvironmentAware, InitializingBean {
    ...
    private static final String OFF = "qinarmy.email.off";

    private static final String SYNC = "qinarmy.email.sync";

    private static final String SIGNATURE = "qinarmy.email.signature";

    private static final String SUBJECT = "qinarmy.email.subject";

    private static final String META_FILE_NAME = "qinarmy.email.meta.fileName";

    private static final String DEFAULT_SUBJECT = "系统信使报信,%s";
    ...
    @Override
    public void send(EmailForm form) throws ParamValidateException {
    ...
    @Override
    public void sendTemplate(TemplateEmailForm form) throws ParamValidateException, EmailException {
        ...
        try {
            ...
            throw new EmailException(ResultCode.EMAIL, e.getMessage(), e);
        }
    }



    private void doSendSimple(final EmailForm form) throws ParamValidateException, EmailException {
        assertEmailForm(form);
        try {
            MailMessage message = createMailMessage(form);
            ...
            if (message instanceof SimpleMailMessage) {
                ...
                throw new EmailException(ResultCode.EMAIL
    ...
    private MailMessage createMailMessage(EmailForm form) throws MessagingException, TemplateEngineException {
        ...
        if (ObjectUtils.isEmpty(to)) {
            LOG.info("没有 找到目标邮箱,不发送邮件,to[{}],prefix[{}]",to, form.getPrefix());
            return null;
        }
    ...
    private String createMeta() {
        ...
        builder.append(now.format(TimeUtils.DATETIME_FORMATTER));
    ...
    private String[] getTo(EmailForm form) {
        return env.getProperty(form.getPrefix() + ".to", String[].class, ArrayUtils.EMPTY_STRING_ARRAY);
    }

    private String[] getCc(EmailForm form) {
        return env.getProperty(form.getPrefix() + ".cc", String[].class, ArrayUtils.EMPTY_STRING_ARRAY);
    }

    private String[] getBcc(EmailForm form) {
        return env.getProperty(form.getPrefix() + ".bcc", String[].class, ArrayUtils.EMPTY_STRING_ARRAY);
    }
     ...
     * 处理附件
     */
    private void processAttachment(List<Triple<String, String, MediaType>> attachmentList, MimeMailMessage message)
        ...
        Resource resource;
        String fileName;
        for (Triple<String, String, MediaType> triple : attachmentList) {
            assertHasText(triple.getFirst(), "file name required");
            assertTrue(triple.getFirst().length() <= 20, "附件名已超过20个字符.");
            assertHasText(triple.getSecond(), "resource pattern required");
            assertNotNull(triple.getThird(), "MediaType required");

            resource = loadResource(triple.getSecond());
            fileName = MimeUtility.encodeText(triple.getFirst(), StandardCharsets.UTF_8.name(), "B");
            helper.addAttachment(fileName, resource, triple.getThird().toString());
    ...
    private void processInline(List<Triple<String, String, MediaType>> inlineList, MimeMailMessage message)
        ...
        Resource resource;
        for (Triple<String, String, MediaType> triple : inlineList) {
            assertHasText(triple.getFirst(), "contentId  required");
            assertNotNull(triple.getThird(), "MediaType required");
            assertHasText(triple.getSecond(), "resource pattern required");

            resource = loadResource(triple.getSecond());

            helper.addInline(triple.getFirst(), resource, triple.getThird().toString());
    ...
    private void assertCommon(EmailForm form) {
        boolean bothEmpty = !StringUtils.hasText(form.getPrefix());
        assertFalse(bothEmpty, "prefix and toKey both are empty");
    ...
    private String createTemplate(TemplateEmailForm form) throws TemplateEngineException {
        ...
        return thymeleafTemplateEngine.renderAsString(engineForm);
    }
    ...
    private String getLocalAddress() {
        String address;
        try {
            address = NetUtils.getPrivateAddr().getHostAddress();
    ...
    private String getDefaultSubject() {
        ...
        if (subject.contains("%s")) {
            subject = String.format(subject, LocalDateTime.now().format(TimeUtils.DATETIME_FORMATTER));
        }
        return subject;
    }
    ...
    @Autowired
    public void setThymeleafTemplateEngine(@Qualifier("thymeleafTemplateEngine")
