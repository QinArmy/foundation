...
public abstract class KeyUtils {
    ...
    protected static final String KEY_BEGIN = "-----BEGIN %s KEY-----";

    protected static final String KEY_END = "-----END %s KEY-----";
    ...
    public static KeyPair createKeyPari(KeyPairType type, int keySize) throws SecurityKeyException {
        try {
    ...
    private static void doWriteToFile(BufferedWriter writer, Key key)
            throws IOException {
        final String base64 = Base64.getMimeEncoder().encodeToString(key.getEncoded()).replaceAll("\\r?\\n?", "");
        //写入上边界
        writer.write(String.format(KEY_BEGIN, key.getAlgorithm()));
        writer.newLine();
        ...
        //写入下边界
        writer.write(String.format(KEY_END, key.getAlgorithm()));
    }
