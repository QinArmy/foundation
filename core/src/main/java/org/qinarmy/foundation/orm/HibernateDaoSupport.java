...
import static org.qinarmy.foundation.util.Assert.assertEquals;
...
public abstract class HibernateDaoSupport implements BaseDao, InitializingBean {
    ...
    public final void afterPropertiesSet() throws Exception {
        ...
        notNull(template, "template required");
        initBean();
    }
    ...
    @Override
    public <D extends IDomain<D>> List<D> find(final Class<D> clazz, final BaseCriteria criteria) {
    ...
    @Override
    public <D extends IDomain<D>> long getCount(Class<D> clazz, final BaseCriteria criteria) {
        return getTemplate().executeWithNativeSession(session -> {
    ...
    @Override
    public <D extends IDomain<D>> boolean isExists(Class<D> entityClass, Serializable id) {
        notNull(entityClass, "entityClass is required.");
        return getTemplate().executeWithNativeSession(session -> {
    ...
    @Override
    public <D extends IDomain<D>> boolean isExistsByUnique(Class<D> entityClass, String propName,
                                                           Serializable uniqueProp) {
        notNull(entityClass, "entityClass is required.");
        notNull(propName, "propName is required.");
        return getTemplate().executeWithNativeSession(session -> {
    ...
    @Override
    public <D extends IDomain<D>> boolean isExistsByUnique(Class<D> entityClass, List<String> propNames,
                                                           List<?> uniqueList) {
        notNull(entityClass, "entityClass is required.");
        notEmpty(propNames, "propNames not empty");
        assertNotNull(propNames, "uniqueProp is required.");
        assertEquals(propNames.size(), uniqueList.size(), "propNames.size not equals uniqueProp.length");
        return getTemplate().executeWithNativeSession(session -> {
    ...
    @Override
    public Serializable save(IDomain<?> domain) {
        return getTemplate().save(domain);
    }
    ...
    @Override
    public <D extends IDomain<D>> D get(Class<D> entityClass, Serializable id) {
        return getTemplate().get(entityClass, id);
    }

    @Override
    public <D extends IDomain<D>> D getByUnique(Class<D> entityClass, String uniquePropName, Serializable uniqueProp)
            throws NotUniqueException {
        notNull(entityClass, "entityClass required");
        notNull(uniquePropName, "uniqueProp required");
        return getTemplate().executeWithNativeSession(session -> {
    ...
    @Override
    public <D extends IDomain<D>> D getByUnique(Class<D> entityClass, List<String> propNames, List<?> uniqueList)
            throws NotUniqueException {
        notNull(entityClass, "entityClass is required.");
        notEmpty(propNames, "propNames not empty");
        assertNotNull(uniqueList, "uniqueProp is required.");
        assertEquals(propNames.size(), uniqueList.size(), "propNames.size not equals uniqueProp.length");
    ...
    private <D extends IDomain<D>> List<Predicate> findPredicate(Root<D> root, CriteriaBuilder builder,
    ...
    protected final <D extends IDomain<D>> Predicate lastIdPredicate(Root<D> root, CriteriaBuilder builder,
                                                                     BaseCriteria criteria) {
    ...
    protected final <D extends IDomain<D>> Order lastIdOrder(Root<D> root, CriteriaBuilder builder,
                                                             BaseCriteria criteria) {
        return Boolean.TRUE.equals(criteria.getAscOrder())
    ...
    protected final <D extends IDomain<D>> void limit(javax.persistence.Query query, BaseCriteria criteria) {
        if (!hasUeLastId(criteria)) {
